				...
				<table class="table table-stripped table-bordered mt-3">
					<thead>
						<tr>
							...
							<th>Trang thai</th>
							<th>Vi tri</th>
							<th>Ngay bat dau sử dụng may</th>
							<th>Giờ sử dụng may</th>
							<th>Thoi gian su dung may</th>
							<th>Mã dịch vụ</th>
					...
					<tbody>
						<c:forEach var="listAll" items="${lisAllDto}">
							<tr>
								<td>${listAll.maKH}</td>
								<td>${listAll.tenKH}</td>
								<td>${listAll.maMay}</td>
								<td>${listAll.trangThai}</td>
								<td>${listAll.viTri}</td>
								<td>${listAll.ngayBatDau}</td>
								<td>${listAll.gioBatDau}</td>
								<td>${listAll.thoiGianSuDung}</td>
								<td>${listAll.maDV}</td>
